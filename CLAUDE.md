...
- **Race:** SF Marathon, July 26, 2026. **Goal: 3:30 (8:00/mi avg on hilly SF course = 3:18 flat-equivalent = VDOT 54) — CONFIRMED as the committed target after the Presidio Pivot test (Jun 21).** The 3:20–3:25 upgrade (VDOT 56–57) was gated on three Presidio criteria; **result: 2 of 3 passed (HR<170 final 3 mi ✓, zero knee comp ✓), time gate MISSED — 1:38:19 elapsed vs sub-1:37:30 (right on the 1:38:20 break-even line).** So **hold 3:30** per the locked rule. **3:25 is deferred, not closed:** the time was dragged by bridge-crowd near-walks + a side stitch + near-zero fueling (1 gel), and he was fuel-limited, not engine-limited — the low final-3-mi HR (154.6) was depletion *suppressing* his HR (per Paul, he couldn't push it higher even trying), not reserve. Revisit a 3:25 upgrade only off a cleaner, well-fueled fitness read later in the build. **The binding marathon constraint exposed Jun 21 is in-race fueling at high HR** (can't get gels down above HR ~155–160) — now the #1 training priority for the back half (see NUTRITION_GUIDE + WORKOUT_LOG Jun 21). Train paces stay at VDOT 54.
...
- **Post-marathon stretch goal (set May 30, 2026):** **Sub-40 10K (6:26/mi for 6.2 mi).** Targeted via a dedicated 12–16 week 10K speed block beginning ~12–16 weeks *after* the SF Marathon (July 26) — i.e. a goal race in **Oct–Nov 2026**. Rationale: a 10K block wants different emphasis (threshold + VO2max, less long-run volume) than the marathon build, so it runs as a separate cycle, not concurrently. Current healthy 10K is ~7:00–7:10/mi (~44–45 min); May 30 PAL 10K was 7:17/mi while sick. Sub-40 needs ~45–50 sec/mi of improvement (~5 min). **Feasibility rests on *demonstrated* fitness, not the Apple VO2max 58** — that optical estimate (flagged June 21) almost certainly overstated his running engine, since his actual races (May 3 5K 21:47, June 21 Presidio HM ~1:38 hilly/under-fueled) read meaningfully lower than a 58 would predict. The honest read: sub-40 is a genuine stretch, not a near-lock — the engine is good-not-exceptional, and **the binding constraints are (a) knee durability under higher-intensity load and (b) actually expressing the fitness in a clean race.** Treat as exploratory — confirm the knee tolerates the SF build first, then assess feasibility post-race off a clean fueled benchmark before committing to the block. 40–42 min is the realistic near-term band; breaking 40 is the stretch.
- **Diet:** Pescatarian. See NUTRITION_GUIDE.md for marathon-specific framework.
- **Location:** Redwood City, CA
- **Gym:** Cañada College (**one consolidated strength + PT day/week, Fri** — restructured Jun 21, 2026; heavy back squat retired, Wed is now an easy run. See TRAINING_PLAN Section V.)
...
**VDOT calibration (May 12, 2026 recalibration):** Training paces anchored to **VDOT 54 flat-equivalent** (3:18 flat marathon = 3:30 SF on hilly course). The May 3 5K (21:47, VDOT ~52) is current fitness; the plan builds toward VDOT 54 over 12 weeks. Training physiology, not race-day output — adaptations happen at actual pace on actual (mostly flat) training terrain, not at SF-course-adjusted average pace. Recalibrate after each benchmark race. **Caveat (flagged June 21, 2026):** the original "VO2max 58 / exceptional engine" premise came from the Apple optical sensor we later found unreliable, and his demonstrated race fitness reads lower than those VDOT labels imply — so treat the VDOT 52/54 figures as *goal-anchored and possibly optimistic*, not a confirmed current measurement. This does NOT destabilize the plan: paces are run-by-HR (the governor self-corrects to true physiology) and the 3:30 goal was empirically validated at Presidio. But the next clean, well-fueled benchmark should re-anchor the VDOT off demonstrated performance rather than any optical estimate.
...
4. Compose a briefing that includes:
   ...
   - **For run days:** state the 5-min pre-run activation (quadruped hip CARs, banded lateral walks, glute bridges) plus the PT-prescribed quadruped banded hip IR (5–10 reps each side). **Lead every run prescription with the HR target — HR is the primary governor for all continuous aerobic running (easy, long, MP, threshold); pace is a secondary readout only (see TRAINING_PLAN "HR is THE governor"). Give the target HR band first, then the pace as the expected readout.** This is terrain-agnostic by design so Paul can run from his house and let pace float with the hills. For track days, write the warm-up jog distance, the interval set with target pace (the one exception — VO2 reps ≤1200m are pace-anchored since HR lags), and the cool-down. For long runs, state the target HR band and any embedded MP segments (by HR) explicitly. **For any run ≥10 mi (or ~75+ min), include a hydration line:** how much water to carry (self-supported default — Bayfront/trail fountains are unreliable), a drink cadence (~4 oz every 15–20 min ≈ 16–20 oz/hr, adjusted down for cool/overcast Bay mornings and up for warm/sunny), and electrolytes (~½ LMNT packet ≈ 500 mg sodium per ~16–20 oz, targeting the NUTRITION_GUIDE ~1000 mg sodium/L). For gut-training long runs, state the Maurten Gel 100 schedule (**re-locked Jun 21 — was PF30; 25 g/gel**: 1 at T−5→0, then every ~25 min, + Maurten Drink Mix sips to reach the 70–80 g/hr band — see NUTRITION_GUIDE progression, and deploy a CAF 100 in the back third on race-pace rehearsals); the low-sodium gels keep the carb separate from the LMNT water by default, so no flask-mixing caveat applies.
   - **For gym days:** include a dynamic warm-up. **Fri Consolidated Strength + PT (the single weekly strength day, restructured Jun 21):** foam roll + 90/90 PAILs 30 sec each + banded lateral walks 10 each direction + Spanish squat hold 30 sec + Copenhagen plank 20 sec each. **Mon Hip Complex Circuit:** the session itself is the warm-up — list the exercises and timing. Then list recommended weights for every weighted exercise from STRENGTH_LOG.md "Current Recommended Weights" section, with sets × reps (Fri strength: 3×8–10 or time-based; Mon: time/rep-based circuit). **NOTE:** the heavy barbell back squat is retired (Jun 21 rebalance) — do not prescribe it; the Wed Heavy Strength day no longer exists (Wed is now an easy run).
   - **Schedule rule (updated Jun 21, 2026 — back-half rebalance):** Strength and running never share the same day. From Week 8 the structure is: **Mon hip circuit, Tue track, Wed easy run, Thu run (medium-long/progression), Fri consolidated strength + PT, Sat shakeout/rest, Sun long run.** This is 5 running days + 1 strength day + Mon mobility (was 4 running days + 2 weighted gym days). The heavy back squat is dropped; the hip/PT rehab work is kept. See TRAINING_PLAN Section V "Back-half rebalance."
