...
- **Type:** Race (half marathon, Pivot test). **Provisional entry — no Strava sidecar yet at log time** (Paul was heading home; the poll hadn't picked it up). **Fold in exact finish time, avg/max HR, final-3-mi HR, and splits from the JSON once it lands** — every number below the gate verdict is athlete-reported pending that.
- **Result (athlete-reported):** **Missed sub-1:37:30 — by "under a minute" per Paul.** Exact time pending sidecar. **Gate verdict: NOT upgraded → hold 3:30 targets.** But read the *why* before reading the clock (below).
