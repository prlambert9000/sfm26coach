...
| 2026-06-09 | Tue | Home-Hills Threshold **+ H10 max-HR test** (compressed) | **Constraint version — home-based, HR-governed, ≤60 min (target ~45–50).** No track drive. ~10 min easy WU jog from door + 3–4 strides. **Threshold: 3 × 5 min @ HR 155–165 (cap 165)** on a rolling/uphill stretch, 90s easy jog between — pace floats on hills, HR governs (~15 min in-zone, trimmed from 4×1600 to fit). 2 min easy. Then the **max-HR test (deferred from May 30): uphill version** — after a short build, **2 × ~75s all-out uphill** walking down to recover, the **2nd absolutely maximal to volitional exhaustion**; keep the watch recording and walk ~30s at the top to catch the peak. 8–10 min easy CD jog home. **Why the test:** every HR zone is anchored to an unconfirmed 180 placeholder (age-based; cleanest H10 reads are two submaximal 167s). **Gates:** knee 1/5 AM (1/5 Mon — set) **and** fully recovered. **Abort:** chest tightness / dizziness / unusual SOB → stop. **Post-test workflow:** read peak from the H10 trace (Strava `max_hr` or Polar Flow); **confirm it's the top of a smooth climb, not a 1-sample artifact** (after a true max effort a high end-peak is REAL). If confirmed, update (1) CLAUDE.md working max, (2) Strava max-HR setting, (3) recompute the HR zone bands. Also grab lowest sleeping/resting HR from Apple Health for Karvonen (HRR) zones. |
| 2026-06-10 | Wed | Heavy Strength (Cañada) | Relative strength benchmarks. **Evening session w/ housekeeper, full Rx — gym fits the 60-min window.** |
| 2026-06-11 | Thu | Steady Base (time-capped ~55 min) | **Constraint version:** run **~50–55 min easy from the door by HR** (≤145 easy / 140–155 steady), hills OK, pace floats — yields ~6–6.5 mi vs the planned 8. Time-capped to fit the ≤60 window; lose ~2 mi of easy aerobic volume, no quality cost. |
| 2026-06-12 | Fri | Stability & Power (Cañada) | Knee tracking focus. **Evening session w/ housekeeper, full Rx — gym fits the window.** |
| 2026-06-13 | Sat | 4 mi Shakeout | 9:30+. **Sonia back — normal morning slot, no constraint.** |
